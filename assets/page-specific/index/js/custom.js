...
async function initGearboxThree() {
  ...
  const canvas = document.getElementById("gearbox-canvas");
  if (!canvas) return;
  ...
  function syncHomepageHeroTitleBranding() {
    ...
    logo.src = "assets/page-specific/index/images/colored-logo.svg";
    logo.alt = "UZHNAQ";
    logo.loading = "eager";
    logo.decoding = "async";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const inStandaloneIndexFolder = false; /* Pages are at root level */
  const pagePrefix = "";
  ...
  function ensureMobileHeaderButtonsStable() {
    ...
    links.innerHTML = [
      ...
      `<a class="site-mobile-header-link" href="${pagePrefix}industries.html">Industries</a>`,
      `<a class="site-mobile-header-link" href="${pagePrefix}machines.html">Machines</a>`,
      `<a class="site-mobile-header-link" href="${pagePrefix}contact.html">Contact</a>`
    ].join('');
  ...
  function initCarouselControlsFallback() {
    ...
    controlsList.forEach((controls) => {
      ...
      const track = root.querySelector('ul');
      const viewport = track && track.parentElement;
      if (!(track instanceof HTMLElement) || !(viewport instanceof HTMLElement)) return;

      const items = Array.from(track.children).filter((node) => node instanceof HTMLElement && node.getAttribute('aria-hidden') !== 'true');
      const prev = controls.querySelector('button[aria-label="Previous"]');
      const next = controls.querySelector('button[aria-label="Next"]');
      const dots = Array.from(controls.querySelectorAll('button[aria-label^="Scroll to page "]'));
      if (items.length < 2 || !(prev instanceof HTMLButtonElement) || !(next instanceof HTMLButtonElement)) return;
  ...
  mapRoot.innerHTML = `
    <div class="site-static-map-frame">
      <img
        ...
        src="assets/page-specific/index/images/map.png"
      ...
      function render() {
        ...
        dots.forEach((dot, dotIndex) => {
          ...
          if (inner instanceof HTMLElement) {
            ...
            inner.style.transform = dotIndex === index ? 'scale(1)' : 'scale(0.88)';
          }
        });
      }
