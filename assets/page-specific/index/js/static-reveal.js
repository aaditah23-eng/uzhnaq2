(function () {
  ...
  function initMapPins() {
    document.querySelectorAll('img[src*="50pPxGlN45r3Ut7lZNgyiMG5g"]').forEach(function (image) {
      ...
      mapRoot.classList.add("static-global-map");
  ...
  function syncMobileTechnologyCarouselSlides() {
    ...
    var donors = sections.map(function (section) {
      ...
      return { section: section, track: track, controls: controls, items: items };
    }).filter(function (entry) {
      return entry.track instanceof HTMLElement && entry.controls instanceof HTMLElement && entry.items.length > 1;
    });

    var donor = donors.reduce(function (best, entry) {
      if (!best || entry.items.length > best.items.length) {
        return entry;
      }
      return best;
    }, null);
    ...
    donors.forEach(function (entry) {
      if (!(entry.section instanceof HTMLElement) || !isRenderable(entry.section)) {
        return;
      }
      if (entry.items.length >= donor.items.length) {
        return;
      }
      if (entry.section.dataset.staticMobileSlidesSynced === "true") {
        return;
      }

      entry.track.innerHTML = donor.track.innerHTML;
      entry.controls.innerHTML = donor.controls.innerHTML;
      entry.section.dataset.staticMobileSlidesSynced = "true";
      if (entry.section.__staticTechCarousel) {
        entry.section.__staticTechCarousel = null;
      }
    });
  }

  function initTechnologyCarousels() {
    syncMobileTechnologyCarouselSlides();
